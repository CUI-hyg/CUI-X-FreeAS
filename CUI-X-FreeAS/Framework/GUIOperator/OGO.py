...
@mcp.tool()
def runcmd(cmd: str):
    """运行指定命令,cmd:命令,这是一个字符串。当可以快捷执行时,请使用该工具。如打开应用等"""
    try:
        pag.alert(text=f'你确定要运行命令吗？\n{cmd}', title='CUISafe - 安全提示', button='OK')
        os.system(cmd)
        return f"命令 {cmd} 已成功执行。"
    except Exception as e:
        return f"执行命令 {cmd} 时出错: {str(e)}"
...
def ImagePosition(userq: str):
    ...
    try:
        local_path = "D:\My_Code\_MyCode_py\AI\CUI-X-HiOS_NEXT\screenshot.png"
