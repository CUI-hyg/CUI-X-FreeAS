...
class MCPClient:
    ...
    def load_prompt(self):
        """加载系统提示"""
        prompt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "MCP_Prompt.txt")
        try:
            with open(prompt_path, "r", encoding="gbk") as file:
                return file.read()
        except FileNotFoundError:
            raise RuntimeError("MCP_Prompt.txt 文件未找到，请确保文件存在。")

    def load_cfg(self):
        """加载MCP配置"""
        mcp_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "mcp.json")
        try:
            with open(mcp_path, "r", encoding="gbk") as f:
                config = json.load(f)
